...
class CloudStore {
  ...
  async saveTicks(ticks) {
    ...
    await this.maybeCleanupTicksOnly();
  ...
  async saveCandleGroup(rows) {
    if (!this.pool || !Array.isArray(rows) || !rows.length) {
      return { ok: true, skipped: true, saved: 0, deleted: 0 };
    }
    ...
    try {
      ...
      const deleteResult = await client.query(
        ...
        where mc.ctid = old_rows.ctid
        `,
        [symbol, timeframe, CANDLE_KEEP_LIMIT]
      );
  ...
  async saveCandleGroupWithRetry(rows) {
    const retries = 3;
    let lastErr = null;

    for (let attempt = 0; attempt <= retries; attempt += 1) {
      try {
        return await this.saveCandleGroup(rows);
      } catch (err) {
        lastErr = err;
  ...
  async saveCandles(candles) {
    ...
    const grouped = this.groupCandlesByKey(clean);
    ...
    for (const rows of grouped.values()) {
      try {
        const result = await this.saveCandleGroupWithRetry(rows);
        saved += result.saved || 0;
        deleted += result.deleted || 0;
      } catch (err) {
        failedGroups += 1;
        this.lastError = err.message || String(err);
        console.warn("[cloudStore] saveCandleGroup:", this.lastError);
      }
    }

    // Тики чистим по расписанию, а свечи уже обрезаны внутри каждой транзакции по symbol + timeframe.
    await this.maybeCleanupTicksOnly();
  ...
  async maybeCleanupTicksOnly() {
    const now = Date.now();

    if (this.cleanupRunning) {
      return { ok: true, skipped: true, reason: "cleanup-running" };
    }

    if (now - this.lastCleanupAt < CLEANUP_INTERVAL_MS) {
      return { ok: true, skipped: true, reason: "cleanup-throttled" };
    }
    ...
    try {
      const result = await this.cleanupTicksOnly();
      this.lastCleanupResult = result;
      return result;
    } catch (err) {
      this.lastError = err.message || String(err);
      console.warn("[cloudStore] cleanupTicks:", this.lastError);
  ...
  async cleanupTicksOnly() {
    ...
    return {
      ...
      deletedTicks: result.rowCount || 0,
      cleanedAt: nowIso(),
    };
  }
